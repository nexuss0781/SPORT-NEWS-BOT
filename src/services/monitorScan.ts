...
import { isPromotionalPost } from "./promo";
...
// Shared core of the monitor: scans source channels for new posts, queues or
...
async function scan(bot: Bot): Promise<MonitorScanResult> {
  ...
  if (!channels.length || targets.length === 0) {
    return { ok: true, processed: 0, message: "No channels or target configured", results };
  }
